 ...
 * when a file system is mounted (see dedupfs_fill_super).
 */
					     ...
					     struct buffer_head ** bh)
{
	...
	if (block_group >= sbi->s_groups_count) {
		dedupfs_error (sb, "dedupfs_get_group_desc",
	...
	desc = (struct dedupfs_group_desc *) sbi->s_group_desc[group_desc]->b_data;
	if (bh)
		*bh = sbi->s_group_desc[group_desc];
	return desc + offset;
}

static int dedupfs_valid_block_bitmap(struct super_block *sb,
					struct dedupfs_group_desc *desc,
					unsigned int block_group,
					struct buffer_head *bh)
{
	...
	if (!dedupfs_test_bit(offset, bh->b_data))
		/* bad block bitmap */
		goto err_out;
	...
	offset = bitmap_blk - group_first_block;
	next_zero_bit = dedupfs_find_next_zero_bit(bh->b_data,
				offset + DEDUPFS_SB(sb)->s_itb_per_group,
				offset);
	if (next_zero_bit >= offset + DEDUPFS_SB(sb)->s_itb_per_group)
		    ...
		    struct dedupfs_reserve_window_node *rsv)
{
	...
	struct dedupfs_reserve_window_node *this;

	while (*p)
	{
		parent = *p;
		this = rb_entry(parent, struct dedupfs_reserve_window_node, rsv_node);
			      ...
			      struct dedupfs_reserve_window_node *rsv)
{
	...
	rsv->rsv_alloc_hit = 0;
	rb_erase(&rsv->rsv_node, &DEDUPFS_SB(sb)->s_rsv_window_root);
}
 ...
 *
 * returns 1 if the end block is DEDUPFS_RESERVE_WINDOW_NOT_ALLOCATED.
 */
 ...
 * and linked to dedupfs inode the first time the open file
 * needs a new block. So, before every dedupfs_new_block(s) call, for
 * regular files, we should check whether the reservation window
 * structure exists or not. In the latter case, this function is called.
 * Fail to do so will result in block reservation being turned off for that
 * open file.
 *
 * This function is called from dedupfs_get_blocks_handle(), also called
...
void dedupfs_init_block_alloc_info(struct inode *inode)
{
	...
	struct dedupfs_block_alloc_info *block_i = ei->i_block_alloc_info;
	struct super_block *sb = inode->i_sb;

	block_i = kmalloc(sizeof(*block_i), GFP_NOFS);
	if (block_i) {
		struct dedupfs_reserve_window_node *rsv = &block_i->rsv_window_node;
		...
		if (!test_opt(sb, RESERVATION))
			...
			rsv->rsv_goal_size = DEDUPFS_DEFAULT_RESERVE_BLOCKS;
		rsv->rsv_alloc_hit = 0;
...
void dedupfs_discard_reservation(struct inode *inode)
{
	...
	struct dedupfs_block_alloc_info *block_i = ei->i_block_alloc_info;
	struct dedupfs_reserve_window_node *rsv;
	spinlock_t *rsv_lock = &DEDUPFS_SB(inode->i_sb)->s_rsv_window_lock;
			 ...
			 unsigned long *pdquot_freed_blocks)
{
	...
	*pdquot_freed_blocks = 0;
	...
	dedupfs_debug ("freeing block(s) %lu-%lu\n", block, block + count - 1);
...
do_more:
	 ...
	 * boundary.
	 */
	if (bit + count > DEDUPFS_BLOCKS_PER_GROUP(sb)) {
		overflow = bit + count - DEDUPFS_BLOCKS_PER_GROUP(sb);
		count -= overflow;
	}
	brelse(bitmap_bh);
			...
			dedupfs_grpblk_t maxblocks)
{
	...
	if (start > 0) {
		 ...
		 * less than DEDUPFS_BLOCKS_PER_GROUP. Aligning up to the
		 * next 64-bit boundary is simple..
		 */
		dedupfs_grpblk_t end_goal = (start + 63) & ~63;
			...
			unsigned long *count, struct dedupfs_reserve_window *my_rsv)
{
	...
	if (my_rsv) {
		...
		if (end > DEDUPFS_BLOCKS_PER_GROUP(sb))
			/* reservation window crosses group boundary */
			end = DEDUPFS_BLOCKS_PER_GROUP(sb);
				...
				dedupfsblk_t last_block)
{
	...
	while (1) {
		...
		rsv = rb_entry(next,struct dedupfs_reserve_window_node,rsv_node);
	...
	if (prev != my_rsv)
		dedupfs_rsv_window_add(sb, my_rsv);
 ...
 *
 */
static int alloc_new_reservation(struct dedupfs_reserve_window_node *my_rsv,
		dedupfs_grpblk_t grp_goal, struct super_block *sb,
		unsigned int group, struct buffer_head *bitmap_bh)
{
	...
	unsigned long size;
	int ret;
	spinlock_t *rsv_lock = &DEDUPFS_SB(sb)->s_rsv_window_lock;

	group_first_block = dedupfs_group_first_block_no(sb, group);
	group_end_block = group_first_block + (DEDUPFS_BLOCKS_PER_GROUP(sb) - 1);
	...
	if (!rsv_is_empty(&my_rsv->rsv_window)) {
		...
		if ((my_rsv->rsv_alloc_hit >
			 ...
			 */
			size = size * 2;
			if (size > DEDUPFS_MAX_RESERVE_BLOCKS)
				size = DEDUPFS_MAX_RESERVE_BLOCKS;
 ...
 *
 * Since dedupfs_try_to_allocate() will always allocate blocks within
...
{
	struct dedupfs_reserve_window_node *next_rsv;
	struct rb_node *next;
	spinlock_t *rsv_lock = &DEDUPFS_SB(sb)->s_rsv_window_lock;
	...
	if (!next)
		...
		next_rsv = rb_entry(next, struct dedupfs_reserve_window_node, rsv_node);
			...
			unsigned long *count, int *errp)
{
	...
	*errp = 0;
	 ...
	 * 0 <= grp_goal < DEDUPFS_BLOCKS_PER_GROUP(sb)
			...
			dedupfsblk_t goal, unsigned long *count, int *errp)
{
	...
	*errp = -ENOSPC;
	...
	dedupfs_debug("goal=%lu.\n", goal);
	/*
	 * Allocate a block from reservation only when
	 * filesystem is mounted with reservation(default,-o reservation), and
	 * it's a regular file, and
	 * the desired window size is greater than 0 (One could use ioctl
	 * command DEDUPFS_IOC_SETRSVSZ to set the window size to 0 to turn off
...
retry_alloc:
	...
	ngroups = DEDUPFS_SB(sb)->s_groups_count;
	smp_rmb();
...
static int dedupfs_group_sparse(int group)
{
	if (group <= 1)
		return 1;
	if (!(group & 1))
...
/**
 *	dedupfs_bg_has_super - number of blocks used by the superblock in group
...
/**
 *	dedupfs_bg_num_gdb - number of blocks used by the group table in group
 *	@sb: superblock for filesystem
 *	@group: group number to check
 *
