		...
		const char *fmt, ...)
{
	...
	va_start(args, fmt);
	printk("%sDEDUPFS-fs (%s): ", prefix, sb->s_id);
	vprintk(fmt, args);
	printk("\n");
	va_end(args);
}
...
void dedupfs_update_dynamic_rev(struct super_block *sb)
{
	...
	es->s_rev_level = cpu_to_le32(DEDUPFS_DYNAMIC_REV);
...
{
	dedupfs_inode_cachep = kmem_cache_create("dedupfs_inode_cache",
					     sizeof(struct dedupfs_inode_info),
					     0, (SLAB_RECLAIM_ACCOUNT|
						SLAB_MEM_SPREAD),
					     init_once);
	if (dedupfs_inode_cachep == NULL)
		return -ENOMEM;
	return 0;
}
...
static int dedupfs_show_options(struct seq_file *seq, struct vfsmount *vfs)
{
	...
	if (test_opt(sb, GRPID))
		seq_puts(seq, ",grpid");
	if (!test_opt(sb, GRPID) && (def_mount_opts & DEDUPFS_DEFM_BSDGROUPS))
		seq_puts(seq, ",nogrpid");
	if (sbi->s_resuid != DEDUPFS_DEF_RESUID ||
	    le16_to_cpu(es->s_def_resuid) != DEDUPFS_DEF_RESUID) {
		seq_printf(seq, ",resuid=%u", sbi->s_resuid);
	}
	if (sbi->s_resgid != DEDUPFS_DEF_RESGID ||
	    le16_to_cpu(es->s_def_resgid) != DEDUPFS_DEF_RESGID) {
		seq_printf(seq, ",resgid=%u", sbi->s_resgid);
	}
	if (test_opt(sb, ERRORS_RO)) {
		int def_errors = le16_to_cpu(es->s_errors);

		if (def_errors == DEDUPFS_ERRORS_PANIC ||
		    def_errors == DEDUPFS_ERRORS_CONTINUE) {
	...
	if (!test_opt(sb, XATTR_USER) &&
	    (def_mount_opts & DEDUPFS_DEFM_XATTR_USER)) {
		...
		u64 ino, u32 generation)
{
	...
	if (ino < DEDUPFS_FIRST_INO(sb) && ino != DEDUPFS_ROOT_INO)
		return ERR_PTR(-ESTALE);
	if (ino > le32_to_cpu(DEDUPFS_SB(sb)->s_es->s_inodes_count))
		return ERR_PTR(-ESTALE);
...
static struct dentry *dedupfs_fh_to_dentry(struct super_block *sb, struct fid *fid,
...
static struct dentry *dedupfs_fh_to_parent(struct super_block *sb, struct fid *fid,
		int fh_len, int fh_type)
{
			    ...
			    int read_only)
{
	...
	if (read_only)
		return res;
	if (!(sbi->s_mount_state & DEDUPFS_VALID_FS))
		dedupfs_msg(sb, KERN_WARNING,
			"warning: mounting unchecked fs, "
			"running e2fsck is recommended");
	else if ((sbi->s_mount_state & DEDUPFS_ERROR_FS))
		dedupfs_msg(sb, KERN_WARNING,
 ...
 */
static loff_t dedupfs_max_size(int bits)
{
	loff_t res = DEDUPFS_NDIR_BLOCKS;
	int meta_blocks;
	loff_t upper_limit;
...
static int dedupfs_fill_super (struct super_block *sb, void *data, int silent)
{
	 ...
	 *       some dedupfs macro-instructions depend on its value
	 */
	...
	def_mount_opts = le32_to_cpu(es->s_default_mount_opts);
	if (def_mount_opts & DEDUPFS_DEFM_DEBUG)
		set_opt(sbi->s_mount_opt, DEBUG);
	if (def_mount_opts & DEDUPFS_DEFM_BSDGROUPS)
		set_opt(sbi->s_mount_opt, GRPID);
	if (def_mount_opts & DEDUPFS_DEFM_UID16)
		set_opt(sbi->s_mount_opt, NO_UID32);
#ifdef CONFIG_DEDUPFS_XATTR
	if (def_mount_opts & DEDUPFS_DEFM_XATTR_USER)
		set_opt(sbi->s_mount_opt, XATTR_USER);
#endif
#ifdef CONFIG_DEDUPFS_POSIX_ACL
   ...
   err = hashcache_init(&(sbi->hc), sbi->hash_cache_size, sbi->hash_len);
	if (err)
		goto failed_mount;
	...
	if (le32_to_cpu(es->s_rev_level) == DEDUPFS_GOOD_OLD_REV &&
	    (DEDUPFS_HAS_COMPAT_FEATURE(sb, ~0U) ||
	     DEDUPFS_HAS_RO_COMPAT_FEATURE(sb, ~0U) ||
	     DEDUPFS_HAS_INCOMPAT_FEATURE(sb, ~0U)))
		dedupfs_msg(sb, KERN_WARNING,
	...
	sb->s_maxbytes = dedupfs_max_size(sb->s_blocksize_bits);

	if (le32_to_cpu(es->s_rev_level) == DEDUPFS_GOOD_OLD_REV) {
		sbi->s_inode_size = DEDUPFS_GOOD_OLD_INODE_SIZE;
		sbi->s_first_ino = DEDUPFS_GOOD_OLD_FIRST_INO;
	} else {
